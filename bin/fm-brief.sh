...
A message relayed to you by the main firstmate is tagged with a leading \`$FM_FROMFIRST_LABEL\` marker followed by an invisible system separator; this marker is untypable, so a human never produces it.
When a message carries that marker and a \`corr=<id>\` token, do the work, then respond via the STATUS/ESCALATION path below, never only in this chat: the main firstmate does not read your chat, so a chat-only reply is lost.
Marked notices without a \`corr=<id>\` token are fire-and-forget instructions; follow them without appending a parent status reply just to acknowledge receipt.
Marked requests with a \`corr=<id>\` token carry a privacy-safe correlation id after the marker; include that exact token in your parent status reply (or in the status pointer to a detailed doc) so the parent can correlate the answer.
...
This is also how you return the answer to a marked from-firstmate request above; there, echo the request's corr token inline:
   \`echo "{state}: corr=<id> {one short line}" >> $STATUS_FILE\`
When that reply belongs to a keyed thread, keep the key next to the verb and the corr token first in the note:
   \`echo "{state} [key=<slug>]: corr=<id> {one short line}" >> $STATUS_FILE\`
...
When a decision you escalated is answered or a blocker clears and your domain resumes, append \`resolved: {how it was decided or unblocked}\`, or \`resolved [key=<slug>]: {how it was decided or unblocked}\` carrying the same key if you opened it with one, so it is durably closed instead of resurfacing behind later unrelated events.
Every key token goes BEFORE the colon, next to the verb: that placement is unambiguous, while a key written inside the note is only read when it leads the note.
...
6. If a decision belongs to a human (product choices, destructive actions),
   ...
   When firstmate replies or a blocker clears and you resume, append \`resolved: {how it was decided or unblocked}\`, or \`resolved [key=<slug>]: {how it was decided or unblocked}\` carrying the same key if you opened it with one, so the decision or blocker is durably closed and does not keep resurfacing.
