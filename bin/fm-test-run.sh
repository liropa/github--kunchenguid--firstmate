...
family_for_basename() {
  case "$1" in
    ...
    fm-parked-decision.test.sh|fm-pi-primary-types.test.sh|fm-promote.test.sh|\
    fm-send-popup-settle.test.sh|fm-send-settle.test.sh|fm-stow-contract.test.sh|\
    fm-supervision-instructions.test.sh|fm-tasks-axi-lib.test.sh|\
    fm-tasks-finished.test.sh|\
    ...
    fm-sbx-gate-vendor.test.sh|fm-sbx-tracked-sync.test.sh|\
    ...
    fm-spawn-settings-drift.test.sh|fm-spawn-worktree-settle.test.sh)
      printf '%s\n' backend-dispatch
      ;;
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-supervisor-target-lib.sh)
      ...
      printf '%s\n' afk
      ;;
    bin/fm-secondmate*|bin/fm-home-seed.sh|bin/fm-backlog-*|\
