...
- **A visible crew** - every crewmate works in a selectable runtime backend you can inspect or steer, with tmux as the hard default and experimental backends documented under `docs/`.
- **Disposable worktrees** - each task runs in an isolated git workspace supplied by treehouse, Orca, or a secondmate-only sandbox backend, so parallel work on one repo never collides.
...
Setup guides for tmux (the default) and every other supported backend (herdr, zellij, Orca, cmux, and secondmate-only sbx) are linked in [Documentation](#documentation) below.
...
```
 ...
 └──┬──────────────┬───────────────┬───┘
    ...
    ▼              ▼               ▼
 ┌────────┐   ┌────────┐      ┌────────┐
 │fm-task1│   │fm-task2│  ... │fm-taskN│   tmux, experimental UI backends, or sbx secondmate sandboxes
...
- [docs/sbx-backend.md](docs/sbx-backend.md) - setup guide for the experimental sbx secondmate backend, plus its signal bridge notes and known gaps.
...
- [docs/claude-settings-trust-posture.md](docs/claude-settings-trust-posture.md) - why branch-carried claude project settings load under the repo-root trust grant, the accepted posture and its evidence, and the dispatch-time drift report.
