...
That path never contacts the network and never touches the gitignored operational dirs, so a secondmate's backlog, projects, and in-flight work are never disturbed.
A linked-worktree home shares the primary's objects and advances immediately.
A standalone-clone home has its own object store, so it is topped up from the primary's path on disk first - objects only, moving no ref - but only when its `origin` resolves to that primary checkout; both shapes therefore converge on the same sweep with no `/updatefirstmate` run.
A home cloned from a real remote instead still has no local source for the commit and is honestly skipped until `/updatefirstmate` refreshes it from origin.
The same launch and the same locked bootstrap sweep also propagate the primary's declared inherited local material into the secondmate's persistent host home: `config/crew-dispatch.json`, `config/crew-harness`, `config/backlog-backend`, `config/herdr-presentation-spaces`, and the one shared captain-preference file `data/captain-shared.md`.
...
If a live home cannot create the per-home inheritance lock because its `state/` is not writable from the current session, bootstrap emits one `SECONDMATE_SYNC:` skip and does not attempt lock stealing or config-reread delivery for that home.
For sbx-backed secondmates, the in-guest home does not receive another copy; [`docs/sbx-backend.md`](../../../docs/sbx-backend.md#guest-home-provisioning-read-through-inheritance) owns the read-through symlink path from the guest clone to the propagated host-home material, and [its "Shared captain delivery" section](../../../docs/sbx-backend.md#shared-captain-delivery-2026-08-09) owns the one exception: `data/captain-shared.md` is delivered to the guest before every agent launch rather than read through that mount.
...
A mid-session `data/captain-shared.md` push is the exception in the other direction: it is delivered at that guest's next agent launch rather than at the push, because the file is never re-read by a running agent, so report it as taking effect then.
