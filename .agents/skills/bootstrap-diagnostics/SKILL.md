...
- `NEEDS_GH_AUTH` - ask the captain to run `! gh auth login` (interactive; you cannot run it for them).
  This line now means no GitHub credential could be produced by any route, so it is a real sign-in problem rather than the sandbox artifact it used to also cover.
