...
Spawn worktree acquisition and launch delivery now follow [Spawn launch delivery](spawn-launch-delivery.md); the optional presentation path does not change that contract.
...
| Send + submit atomically | `herdr pane run <pane> <command>` | Runs and submits a command in one call; used for the verified spawn launch described in [Spawn launch delivery](spawn-launch-delivery.md). |
...
Other runtime backends, including zellij, orca, cmux, and sbx, are not yet supported as supervisor backends - the daemon refuses loudly at startup (`FM_SUPERVISOR_SUPPORTED_BACKENDS="tmux herdr"`) rather than misapplying tmux primitives to a pane that isn't a tmux pane.
...
- **RESOLVED: worktree-discovery isolation guard's symlinked-project-prefix false refusal.** Originally discovered while building the runtime-backend-auto-detection real smoke test (`tests/fm-backend-autodetect-smoke.test.sh`), which needed a scratch project.
  ...
  Fixed 2026-07-06 (backlog `fm-spawn-symlink-guard-s8`): that implementation canonicalized `PROJ_ABS` and each observed pane cwd before the worktree-discovery comparison.
  The current off-pane acquisition contract is in [Spawn launch delivery](spawn-launch-delivery.md).
