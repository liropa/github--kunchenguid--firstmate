...
It runs only in a primary home, and it denies only an executed `bin/fm-*.sh` command other than `bin/fm-wake-drain.sh`, `bin/fm-watch-arm.sh`, or the ordinary literal `bin/fm-teardown.sh` when task metadata is in flight and that home's watcher lock is not verified healthy.
...
The existing `bin/fm-turnend-guard.sh` Stop integration remains the final backstop.
