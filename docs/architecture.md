...
Both supervisors therefore watch the attributed run's own step logs rather than only the pane: an advancing run resets the timer, while a run that stops writing still escalates on the unchanged schedule and is named as a stalled run instead of a wedged pane.
