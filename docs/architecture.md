...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or an X-mode mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their attributed pipeline advancing, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
A crew blocked inside a blocking validation gate call idles by construction, because the call blocks while the pipeline works elsewhere, so pane capture alone cannot tell it from a wedged crew.
Both supervisors therefore watch the attributed run's own step logs rather than only the pane: an advancing run resets the timer, while a run that stops writing still escalates on the unchanged schedule and is reported as not advanced rather than diagnosed as stalled.
Crews without an attributed host-visible run, including sbx crews whose gates run in the guest, keep the prior pane-only behavior.
Repeated provably-working stale escalations on the same pane without a busy signature add an escalation count to the wake reason and, at `FM_WEDGE_DEMAND_INSPECT_COUNT`, a `demand-deep-inspection` marker.
Those actionable wakes are written to a durable local queue (`state/.wake-queue`) before detector state advances, so a missed process exit can be recovered by draining the queue.
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign only when `bin/fm-crew-state.sh` reports positive evidence that the crew is still working: an actively running no-mistakes step attributed under its run-head rules or a backend busy signature.
...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it attributes a no-mistakes run, active or terminal, under its branch and run-head rules, then keeps that run-step authoritative even if the pane has closed.
A run head that this worktree cannot resolve at all also binds on the branch alone, because the gate authors its fix commits in its own object store, and the emitted detail records that weaker provenance.
...
Claude keeps its tracked background-task protocol and adds a narrow PreToolUse continuity gate that allows drain, arm recovery, and fail-closed teardown while refusing only other fleet commands when tasks are in flight and the home watcher lock is not verified healthy.
The existing turn-end guard remains the final backstop for all five harness protocols.
The arm wrapper's `--restart` mode signals only the watcher recorded in the current home's `state/.watch.lock`, so restarting one home cannot kill sibling secondmate watchers.
...
On every verified primary harness, tracked hook integration gives the primary session a push-based backstop: when work is in flight and no verified healthy watcher lock with a fresh beacon is live, direct Stop hooks block and passive turn-end hooks force one bounded follow-up.
...
That library also owns the run-progress witness both supervisors use to separate a gate-blocked crew from a wedged one, so the discrimination cannot drift between the two triage paths.
...
The [tmux submit acknowledgement contract](tmux-backend.md#submit-acknowledgement-landed-is-empty-with-two-busy-queue-shapes) owns queued-Enter handling and its limits.
...
Runtime auto-detection is innermost-first: `$TMUX` wins over `HERDR_ENV=1`, which wins over cmux's primary `CMUX_WORKSPACE_ID` marker and documented fallback signals; auto-detected herdr or cmux prints a one-time opt-out notice, auto-detected tmux stays silent, and zellij, orca, and sbx are never auto-detected (only explicit selection).
...
`fm-watch.sh` polls each window's backend for a busy state: tmux, zellij, orca, cmux, and sbx have no native primitive and always report unknown, preserving the original pane-tail-regex detection unchanged; herdr's `agent.get` semantic state (working/idle/done/blocked) is consulted first for stale detection, with unknown native states falling back to the same regex.
...
For capable herdr sessions, the same watcher replaces its terminal sleep with a bounded native event wait that detects `blocked` immediately and confirms it over a dwell before escalation; [herdr-backend.md](herdr-backend.md#native-paneagent_status_changed-push-escalation-sub-second-blocked-detection) owns the mechanism, capability gates, and verification evidence.
The deeper session-start agent-process liveness probe is separate from that busy-state poll: tmux, herdr, and sbx have verified classifiers for secondmate recovery, while zellij, Orca, and cmux currently report `unknown` rather than guess.
...
sbx is experimental, explicit-only, and secondmate-only: each secondmate runs in a clone-mode Docker Sandbox microVM with an in-guest tmux agent, while firstmate supervises it through a bind-mounted signal directory.
Its adapter contract, liveness mapping, signal bridge, resurrection path, live verification status, and remaining caveats are recorded in `docs/sbx-backend.md`.
Codex App support is recorded in `docs/codex-app-backend.md`; it is not selectable as a runtime backend.

## Worktrees, not branches in your checkout

Tasks never intentionally touch your project clone; [treehouse](https://github.com/kunchenguid/treehouse) pools clean worktrees for tmux, herdr, zellij, and cmux tasks, Orca creates its own worktrees for `backend=orca`, and sbx secondmates run from clone-mode sandbox copies.
...
In lock-refused read-only mode, both surfaces keep the alarm but switch to read-only wording with no repair command.
...
When called with `FM_HOME=<this-firstmate-home>` or when `FM_HOME` is already set to the active firstmate home, metadata-routed `fm-send.sh` traffic to a live `kind=secondmate` is prefixed with the from-firstmate marker from `bin/fm-marker-lib.sh`, so correlated requests return terse answers through status lines and detailed answers through docs plus status pointers instead of replying only in chat.
Normal marked sends carry a correlation token and are guarded against a missing correlated report without reading the secondmate conversation; `bin/fm-send.sh` owns the fire-and-forget notice exception, and `bin/fm-pending-reply-lib.sh` owns the correlation, recovery, escalation, and retention contract.
...
After seeding a secondmate, `fm-backlog-handoff.sh` validates the fleet-specific handoff, then atomically delegates already-judged in-scope queued item moves to `tasks-axi mv` so the domain queue starts in the right place; [`docs/sbx-backend.md`](sbx-backend.md#backlog-handoff-signal-bridge-batches) owns the sbx signal-bridge variant.
Idle secondmate panes are healthy; teardown is explicit and refuses while the secondmate home has in-flight work unless the captain has approved discard with `--force`.

Secondmate host homes converge conservatively to the primary's version and declared inherited local material at launch and during locked session start.
The [`secondmate-provisioning` skill](../.agents/skills/secondmate-provisioning/SKILL.md) owns the full guarded sync, propagation, nudge, and mid-session local-material push contract; [`docs/sbx-backend.md`](sbx-backend.md#guest-home-provisioning-read-through-inheritance) owns sbx's in-guest read paths and delivery timing.
...
The firstmate-internal [`stow` skill](../.agents/skills/stow/SKILL.md) owns the task-note update procedure.
