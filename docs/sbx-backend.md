...
**Presence means newly appeared, not merely visible**: the needle is the steer's first 24 chars (marker + a few payload chars), which a *previous* steer's rendered line inside the 30-line capture window also matches - so the occurrence count is baselined after the ready poll and before typing, and only a count above the baseline reads as parked composer text (one extra capture exec per steer). A newly observed post-Enter busy footer with the needle visible still confirms submission, because the new line may replace an old same-prefix line in the bounded capture window. Without this, a resume-time swallow behind a stale same-prefix line converts the designed retype into a no-op Enter loop and the steer is lost behind a clean exit (observed live, 5-secondmate soak: 1 of 5 concurrent resurrections).
