...
```
2026-08-11T18:22:31Z released-idle pin=743s arm1=0 arm2=0 arm3=0 arm4=0 arm5=0 crew=1 panes=2 status=1785512635 nmlog=1786127662
```
...
- **An UNRECORDED park still reads idle** - the parked-decision arm ("Parked-decision arm" above) fires on the durable record of a wait: a `needs-decision` or `blocked` line the worker actually appended. A worker stopped on something it never reported - a harness sign-in prompt, an auth wall it did not classify - leaves no such record, so all five arms read 0 and the VM still stops on top of it. Separating that from a genuinely finished worker needs pane-content capture, which reads a screen rather than a record and so carries the false-positive risk every screen-reading arm carries; it was deliberately left out of the 2026-08-11 change rather than bolted on without evidence. The shape to watch for is a `released-idle` line carrying `crew=1` and `arm5=0` for a task that later turns out to have been waiting on a human.
