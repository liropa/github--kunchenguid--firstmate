...
**What the worker proved, and what it did not**: every check above this line is hermetic.
The worker that wrote the fix created, started, and exec'd no sandbox, and reproduced nothing against the real sbx API.
...
The sandbox that read `stopped` before the fix reads `running` after it.
A silent send is not proof of delivery on its own, so firstmate also observed `state/.sbx-delivered-agent-dotfiles` refreshed at 18:02:55, and read the guest pane with `sbx exec fm-agent-dotfiles -- bash -lc 'tmux capture-pane -p -t fm:0'` to see the marked message `[fm-from-firstmate]corr=2deb73c7807a1a35` arrive and the secondmate working on it.
The same command, against the same sandbox, refused every resurrection before the fix and completed one after it: the real sbx API rejection is reproduced, and the real recovery is confirmed.
...
All three of those, however, read a terminal screen or a status mtime, which is why a fourth arm was added on 2026-08-07 after four validation runs died to exactly that limit ("Gate-activity arm" below), and a fifth on 2026-08-11 for the one wait that no screen or mtime can express at all ("Parked-decision arm" below).
A release that no arm can explain is separately classified on the way out, so the retained log says whether the static screen was a sign-in prompt or a finished worker ("Static-screen classification" below); that classification is evidence only and changes no release.
...
**That breadcrumb lives on the signal-bridge mount, at `$FM_SBX_SIGNALS_ROOT/<id>/<id>.guest-active` (default `~/dev/fm-signals/<id>/<id>.guest-active`) - never in the primary's `state/`.**
Both writer and readers agree on the mount path (`bin/backends/sbx.sh`'s keep-alive loop derives it from the same `turn-ended` mount file; `bin/fm-watch.sh`'s stranding suppression stats it there), so `state/<id>.guest-active` never exists and its absence proves nothing.
Reading it at the wrong path is a live diagnostic trap, not a hypothetical one: on 2026-07-30 an absent `state/<id>.guest-active` was taken as evidence that no keeper had been armed, that unsupported conclusion was reported to the captain, and it propagated verbatim into a later task brief and the Done archive before the 2026-08-03 investigation caught it.
When the exec ends, the keep-alive's host-side wrapper classifies the outcome: an idle release or an idle cap expiry remains alarm- and control-flow-silent but leaves the verdict-log evidence described below, while a cap expiry with work still active - or an exec death while the breadcrumb was fresh (an explicit `sbx stop`, a crash) - waits `FM_SBX_MIDTASK_STOP_SETTLE` (default 120 s, covering the measured auto-stop grace), reads the sandbox state once (the only sbx CLI cost, spent per rare suspicious exit, never per poll), and on `stopped`/`absent` records the `state/.sbx-midtask-stop-` marker the watcher surfaces as a named mid-task-stop alarm (below).
...
### Parked-decision arm (the fifth arm, 2026-08-11)

The first four arms all read **activity**: a redrawing pane, a moving mtime, a log being written.
A worker waiting for the captain to answer a question produces none of it, and that is not a gap in the arms - it is the shape of the event.
A parked worker and a finished one are the same static pane and the same stale status file, which is exactly why the previous contract could refuse to pin for an idle-parked TUI and be right to.
The fifth arm reads the **record of the wait** instead: while a crewmate task is registered, it asks whether any live in-guest task is still sitting on a decision nobody has answered.

**The evidence.**
<!-- fm-authority: firstmate-observation 2026-08-11 - the keeper verdict log lives in a gitignored private home; these lines were read there and cannot be reproduced from this checkout -->
The keeper's own verdict log recorded three stops on 2026-08-10 (`state/.sbx-keepalive-agent-dotfiles.log`):

```
2026-08-10T21:12:12Z released-idle pin=1736s arm1=0 arm2=0 arm3=0 arm4=0 crew=1 panes=2
2026-08-10T21:52:56Z released-idle pin=1286s arm1=0 arm2=0 arm3=0 arm4=0 crew=1 panes=2
2026-08-10T22:16:59Z released-idle pin=373s  arm1=0 arm2=0 arm3=0 arm4=0 crew=1 panes=2
```

Each line proves a keeper was armed and released on its own decision, with a crewmate registered and all four arms reading zero, while the in-guest worker was parked on a decision firstmate had escalated to the captain.
<!-- /fm-authority -->
This is not the superseded in-guest-sub-worker gap (fork issue #12): all four arms were live for these stops, and the fourth had already shipped.
`data/learnings.md`'s 2026-08-09 entry names the mechanism in one line - an escalation to the captain *is* the idle window.

**The source of truth is one that already exists, and it is already inside the guest.**
`bin/fm-classify-lib.sh`'s keyed open/resolved fold over a worker's status stream is this repo's single statement of when a decision is open: a `needs-decision`/`blocked` line opens a keyed decision and only an explicit resolution or a verified captain-hold transfer closes it.
The guest home is a firstmate clone with its own `bin/` (`bin/fm-home-seed.sh` refuses one without it), so the keeper runs the guest's own `bin/fm-parked-decision.sh`, which composes that fold rather than reimplementing it.
Nothing new is published and nothing has to be cleared by hand - **delivering the answer clears the pin** because the worker's own resolution line closes the decision, which is a contract `bin/fm-brief.sh` already writes into every brief.

**Why not a host-published marker.**
The obvious alternative - firstmate touches a marker on the signal-bridge mount when it escalates and removes it when it answers - loses the race and adds a second source of truth.
The keeper polls every `FM_SBX_KEEPALIVE_POLL` (default 5 s) and releases on the first poll where the turn has ended and no arm reads work, whereas the watcher polls every 15 s, `wake()` **exits** the watcher on any actionable signal, and the marker could not be written until firstmate had been woken, had handled the escalation and had re-armed supervision.
It would also depend on firstmate remembering both halves; the in-guest read depends on nothing the fleet does not already do.
`bin/fm-decision-hold.sh` was checked first, as the other durable decision record, and does not fit: it is scoped to decisions an investigation or visual review surfaces, its holds live in the originating home's backlog rather than on any path the guest can read, and an ordinary `needs-decision` park never creates one.
It does share the terminal-event rule, so that rule moved into `bin/fm-classify-lib.sh` beside the fold and both callers now read it from there.

**What it will not do.**
It never pins for a finished worker: the predicate reports nothing once a task's newest event is `done` or `failed`, so a decision line nobody explicitly resolved cannot convert cleanup into a permanent pin.
It is gated on a registered crewmate exactly like the pane-change arm, so the ordinary idle secondmate's auto-stop behaviour is untouched - and the gate is the right one, because the in-guest crewmate is the thing a stop destroys that resurrection cannot rebuild.
It adds **no `sbx exec` to the poll path**: it is a pure in-guest read inside the keeper's already-open exec, and it creates, moves and touches nothing.
It does **not** stamp the `<id>.guest-active` breadcrumb, unlike arms 1-4: the breadcrumb means in-guest work is active and the beacon's stranding arm accepts a fresh one as acknowledgement of a delivery, so letting a park refresh it would silence that alarm for a guest that never processed the answer it was sent.
A guest home cloned before this shipped has no predicate to run; the arm then reads 0 and behaviour is exactly what it was, converging when the guest clone fast-forwards ("Tracked-file sync" above).

**The bound, stated explicitly.**
A parked pin is bounded by `FM_SBX_KEEPALIVE_MAX` (default 7200 s) like every other arm, and ends as `capped-active` when it expires.
Nothing re-arms a keeper except a launch or a turn-submitting steer, so an unanswered decision holds the VM for **at most one cap window from the last steer** and then lets it stop; it can never pin a machine indefinitely.
A firstmate that dies mid-escalation leaves no marker behind to strand the guest awake, because there is no marker - the same bound applies and nothing outlives it.

Verified against fixtures in `tests/fm-backend-sbx.test.sh` and `tests/fm-parked-decision.test.sh`, in both directions: a parked worker pins where every other arm reads idle; the same worker releases once the answer is recorded; a finished worker's unresolved decision line never pins; an open decision with no registered crewmate never pins; a parked pin leaves the breadcrumb untouched while a busy pane on the same fixture still stamps it; and an older guest home with no predicate behaves exactly as before.
Run against the pre-fix loop the first two reproduce the incident exactly - `not ok - a worker parked on an unanswered decision must pin the VM, not read as idle`, with the loop reporting `arm1=0 arm2=0 arm3=0 arm4=0 crew=1`, and `not ok - the keeper released on the turn-end while a worker sat on an unanswered decision (unexpected: 'fm-keepalive released-idle')`, which is the logged verdict itself.
The two release-preserving fixtures (`a finished worker awaiting cleanup` and `a genuinely idle guest still releases on the turn-end`) were confirmed passing **before** the change as well as after, so they pin a property rather than record a fix.
Not verified end to end against a real sandbox VM: the fleet's live sandbox holds a secondmate's private records, the same scope limit the fourth arm carries.

### Static-screen classification (`park=`, 2026-08-11)

The parked-decision arm above fires on the durable record of a wait, so it covers only a park the worker actually **reported**.
Two guests that report nothing remain identical to all five arms, and their remedies are opposite:

- a worker stopped on a **harness sign-in prompt**, which posts no status at all, so there is no open decision for the fifth arm to find - a human has to restore the login before anything moves again; and
- a worker that has **finished** and is waiting for cleanup, which is correctly allowed to let the machine stop - nothing is needed.

Both are a static pane with every arm at zero, and until this shipped the retained record could not tell them apart.
The keeper now classifies which it was and reports it as one more fixed field on the detail line, so the log alone answers the question after the machine is gone.

`park=` is a closed four-value enum:
...
The classification is computed in `emit` immediately before the loop exits and is read by no condition: not a pin, not a release, not the `guest-active` breadcrumb.
An unclassifiable guest still stops exactly as it did, and nothing here is a new reason a guest cannot idle down.

**It records a classification, never the pane text - a security property, not a preference.**
The verdict log lives on the signal-bridge mount, which survives both the stop and a full guest rebuild, and a worker pane can hold credential material (a token, `gh auth` output, an OAuth error carrying a value).
Only one token from the closed enum crosses the boundary, and the host wrapper's whitelist admits nothing else, so a guest emitting `park=` followed by free text has its whole report dropped rather than logged.

**The signature set is evidence-backed and deliberately small.**
`FM_SBX_AUTH_REGEX` (`bin/backends/sbx.sh`) defaults to `Please run /login`, `Not logged in`, `OAuth access token has expired`, `401 Unauthorized`, and `sign(ing)? in again`.
Every alternative is an observed shape rather than an invented one: `Not logged in` / `401` is the in-guest daemon failure recorded under "Guest shell-profile env" above (2026-07-23), and `401 Unauthorized` plus the codex wrong-flavor body's *Please try signing in again* are both in the "Agent flavor vs driver harness" matrix above (2026-07-27).
...
**Storage is the existing bound, and nothing new is retained.**
The verdict is one token on a line the keeper was already writing, in a file already trimmed to the newest `FM_SBX_KEEPALIVE_LOG_LINES` (default 200) at twice that count - no new file, no new retention scheme.

A guest home cloned before this shipped has no `bin/fm-tasks-finished.sh` to run; `finished` then cannot be asserted and the classification reports `unknown`, converging when the guest clone fast-forwards ("Tracked-file sync" above).
Verified against the fixtures in `tests/fm-backend-sbx.test.sh` and `tests/fm-tasks-finished.test.sh`: the recorded sign-in pane classifies as `auth`, a finished worker as `finished`, an unrecognised static pane as `unknown`, a working guest and a guest with no registered task as `none`; a pane carrying a credential beside the matching line still emits only the enum; a detail line carrying the new field passes the wrapper's whitelist while a free-text verdict is refused; the classification leaves the verdict itself unchanged; and an older guest home with no predicate reports `unknown` rather than claiming `finished`.
Run against the pre-classification loop every one of those reproduces as a missing field (`not ok - a pane holding the sign-in signature must be recorded as a sign-in park (missing: 'park=auth')` and its siblings).
Not verified end to end against a real sandbox VM: the fleet's live sandbox holds a secondmate's private records, the same scope limit the fourth and fifth arms carry.
...
```
2026-08-11T18:22:31Z released-idle pin=743s arm1=0 arm2=0 arm3=0 arm4=0 arm5=0 crew=1 panes=2 park=finished status=1785512635 nmlog=1786127662
```

The verdict is one of `released-idle`, `capped-idle`, `capped-active`, or `no-verdict` (the exec died without printing one), `pin=` is how long the host held the connection, and the trailing fields are the guest loop's own report of the deciding poll: how each of the five arms read (`arm5` is the parked-decision arm, the only one that reports a wait rather than activity), whether a crewmate task was registered (`crew`), how many tmux panes the guest had (`panes`), why the screen was static (`park`, "Static-screen classification" above), and - on `released-idle` with a task registered - the newest `*.status` mtime under the guest home's `state/`, plus the newest gate-log mtime whenever one was read (`nmlog`).
...
It is written host-side after the exec has already ended, so it can reach neither the keeper's decisions nor the guest, and every write failure is swallowed - an unwritable log still leaves the mid-task-stop classification intact.
...
- **Pass this probe** only for a clean tree whose every commit is on a remote (a fork counts), OR a confirmed-**absent** sandbox (already gone, nothing to lose).
...
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header and `--help` own the export requirement and the separate `--force --discard-private` exception.
[`fm_backend_sbx_export_private` in `bin/backends/sbx.sh`](../bin/backends/sbx.sh) owns the archive layout, host verification, checksum mark, and stopped-guest contract.
...
1. **Stop guest writers and export the guest records.**
   ...
   Without explicit discard authority, a failed export must never proceed to removal.
   It leaves the guest stopped, with its disk and saved agent session intact.
...
2. **Verify, remove the old VM, and spawn with restoration enabled.**
   ...
   The agent reads the restored records on its first turn; [`bin/fm-spawn.sh`](../bin/fm-spawn.sh)'s header and `--help` own the restore input, ordering, and failure contract.
...
- **A run whose driver stops polling can still go silent past the activity window** - this is what is left of the silent-worker gap after the gate-activity arm ("Gate-activity arm" above) closed the proven failure shape. The arm sees a validation run only while something under `$HOME/.no-mistakes/logs/` is being written, and that depends on the driver: in the four fatal 2026-08-05/07 releases the in-guest worker polled `get_run` about four times a second so the daemon log was continuously hot, but on the host the same no-mistakes version left the whole log tree untouched for **57 minutes** mid-run (measured 2026-08-07, run `01KZEKQ9BHQ23KNRM84RY96KF6`: a 3415 s daemon-log gap, with the step log written once at completion rather than streamed). A step that neither streams its log nor is polled therefore still reads idle on all five arms unless it is also parked on an open decision. Widening this would mean a genuine in-guest **process** probe - the thing the four standing objections were about, three of which the guest-side placement already answers ("Gate-activity arm" above) - so it is cheap to reconsider, but it should wait for a recorded occurrence rather than be built on this inference. The shape to watch for is a `released-idle` line carrying `crew=1`, all five arms 0, and an `nmlog=` that is stale but recent enough to bracket a run.
- **An UNRECORDED park still reads idle, and still stops the machine** - the parked-decision arm ("Parked-decision arm" above) fires on the durable record of a wait: a `needs-decision` or `blocked` line the worker actually appended. A worker stopped on something it never reported - a harness sign-in prompt, an auth wall it did not classify - leaves no such record, so all five arms read 0 and the VM still stops on top of it. **The diagnosability half of this closed on 2026-08-11**: `park=` ("Static-screen classification" above) now records whether the static screen was a sign-in prompt, a finished worker, or neither, so the retained log alone separates the two opposite remedies after the fact. What remains open is the *pin*: a sign-in park is a screen reading, and pinning on it would mean letting a screen match hold a VM awake, which is the false-positive class every screen-derived alarm in this document is refused on. It is left deliberately as instrumentation until a recorded stop shows the pin would have saved work that the classification alone did not. The shape to watch for is a `released-idle` line carrying `crew=1`, `arm5=0` and `park=auth`.
