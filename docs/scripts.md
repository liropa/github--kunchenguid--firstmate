...
| `fm-update.sh`           | Fast-forward-only self-update of firstmate, secondmate homes, and sbx guest clones   |
...
| `backends/sbx.sh`        | Experimental secondmate-only Docker Sandboxes adapter                                |
...
| `fm-marker-lib.sh`       | Shared from-firstmate marker, detector, and idempotent transformation                 |
| `fm-pending-reply-lib.sh` | Parent-owned secondmate pending-reply expectations, recovery, escalation, and stale reconciliation |
...
| `fm-tasks-axi-lib.sh`    | Shared backlog-backend selector, `tasks-axi` compatibility probes, and explicit-path wrapper |
...
| `fm-pr-check-migrate.sh` | Quarantine unsafe task polls without execution, distinguish missing metadata identity, and rebuild only canonical polls |
