...
Before printing, the wrapper reads the first line of `state/.lock` as a bare pid and walks at most eight parents from its own pid, matching `bin/fm-lock.sh`'s primary ancestry path and Pi's `lockOwnership()` ancestry depth.
