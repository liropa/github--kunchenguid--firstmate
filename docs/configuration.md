...
FM_PANE_CYCLE_MEMORY=4             # recent distinct captures the watcher remembers per window; a pane that only cycles among them has produced no new output and is not progressing, so an animated pending-tool-call glyph cannot masquerade as work. 1 restores the pre-2026-08 byte-identical gate; docs/tmux-backend.md owns the measurement and the two limits
