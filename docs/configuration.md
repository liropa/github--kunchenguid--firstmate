...
FM_PUSH_BLOCK_DWELL=90             # seconds a pane on a backend's native push path must stay blocked before that block escalates; a block anything services inside the window cancels its own escalation, so the alarm tracks how stuck a crew is rather than how many approval prompts it raised. docs/herdr-backend.md owns the behavior
