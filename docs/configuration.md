...
FM_SBX_DELIVERY_ACK_SECS=900   # sbx-only: seconds of silence after a delivery (no turn end, status line, or in-guest work) before the same sbx-stranded wake names an unacknowledged delivery; 0 disables this stranding arm, which is the only one that can see a guest producing no turn-ends at all; docs/sbx-backend.md owns the contract
