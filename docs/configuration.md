...
`tmux` is the verified reference backend (see [`docs/tmux-backend.md`](tmux-backend.md)); `herdr`, `zellij`, `orca`, `cmux`, and `sbx` are experimental spawn backends (see [`docs/herdr-backend.md`](herdr-backend.md), [`docs/zellij-backend.md`](zellij-backend.md), [`docs/orca-backend.md`](orca-backend.md), [`docs/cmux-backend.md`](cmux-backend.md), and [`docs/sbx-backend.md`](sbx-backend.md)).
Treehouse remains the worktree provider for tmux, herdr, zellij, and cmux, since herdr, zellij, and cmux are session providers only; Orca provides both the task worktree and terminal endpoint; sbx provides a clone-mode Docker Sandbox microVM for secondmate homes only.
...
Zellij, Orca, and sbx are never auto-detected; select them by putting the name in a local `config/backend` file, by exporting `FM_BACKEND=<name>`, or by telling the first mate in chat.
Any value other than `tmux`, `herdr`, `zellij`, `orca`, `cmux`, or `sbx` is rejected until another adapter is implemented and verified.
`fm-spawn.sh` accepts `tmux`, `herdr`, `zellij`, `orca`, and `cmux` for ship and scout tasks; `backend=sbx` refuses ship and scout tasks because it is secondmate-only.
For secondmate spawns, `tmux`, `herdr`, `zellij`, and `sbx` are accepted, while `backend=orca` and `backend=cmux` both still refuse until secondmate launch semantics are designed for each.
...
Today that probe can classify tmux, herdr, and sbx secondmate endpoints as `alive`, `dead`, or `unknown`; zellij, Orca, and cmux report `unknown` until their own agent-process classifiers are verified.
...
sbx creates one clone-mode Docker Sandbox named `fm-<id>` per secondmate and supervises it through a bind-mounted signal directory whose host symlinks preserve the normal `state/<id>.status` and `state/<id>.turn-ended` scan paths.
...
A standalone-clone home reaches that same state on its own next local sync when it was cloned from this primary checkout, and otherwise receives the rule through `/updatefirstmate`'s origin refresh instead.
...
That delta is owned in code by `fm_backend_required_tools` in `bin/fm-backend.sh`: the resolved backend's own session-provider CLI (`tmux`, `herdr`, `zellij`, `orca`, `cmux`, or `sbx`), `jq` for the JSON-emitting experimental adapters (`herdr`, `zellij`, `cmux`, `sbx`) whose spawn or liveness paths parse the backend's JSON output, and the `treehouse` worktree provider for every host session-provider-only backend (`tmux`, `herdr`, `zellij`, `cmux`).
...
The locked session-start bootstrap step also runs the guarded local secondmate sync for recorded live secondmate homes, runs the sbx guest tracked-file sync for recorded sbx secondmates, then propagates declared inherited local material into each validated live home.
It emits `SECONDMATE_SYNC:` only when a home or sbx guest was skipped for an actionable sync reason, inheritance failed, or a divergent shared captain-preference copy was quarantined.
...
FM_SBX_GUEST_ACTIVE_WINDOW=120   # sbx-only: seconds an in-guest child signal or no-mistakes log mtime counts as work for keep-alive release, and a guest-active breadcrumb suppresses stranding; docs/sbx-backend.md owns the behavior
FM_SBX_MIDTASK_STOP_SETTLE=120   # sbx-only: seconds the keep-alive wrapper waits after a suspicious exit before one sandbox state read for mid-task-stop classification; docs/sbx-backend.md owns the alarm contract
FM_SBX_KEEPALIVE_LOG_LINES=200   # sbx-only: lines kept when the per-task keep-alive verdict log is trimmed; 0 disables trimming. Evidence only - nothing reads this file and no alarm derives from it; docs/sbx-backend.md owns the contract
FM_SBX_AUTH_REGEX='Please run /login|Not logged in|OAuth access token has expired|401 Unauthorized|sign(ing)? in again'   # sbx-only: harness sign-in signatures matched case-insensitively against a guest pane tail, so a keep-alive verdict line records WHY a static screen was static. Instrumentation only - it decides no pin, release, or breadcrumb, and only a closed enum reaches the log, never pane text; docs/sbx-backend.md owns the contract
...
FM_CAPTAIN_RE='done:|awaiting-validation:|needs-decision:|blocked:|failed:|PR ready|checks green|ready in branch|merged'   # captain-relevant status regex; nonterminal progress verbs remain excluded even when their prose matches. An override replaces the WHOLE set, so a custom value that omits awaiting-validation: makes a crew waiting on its validation trigger invisible to supervision
FM_CLASSIFY_PAUSED_VERB=paused     # leading status verb for a declared external wait; excluded from FM_CAPTAIN_RE and distinct from blocked
FM_PANE_CYCLE_MEMORY=4             # recent distinct captures the watcher remembers per window; a pane that only cycles among them has produced no new output and is not progressing, so an animated pending-tool-call glyph cannot masquerade as work. 1 retains only the immediately preceding capture, so a capture is progress unless it is identical to the one before it; docs/tmux-backend.md owns the measurement and the two limits
FM_BLIND_CAPTURE_POLLS=3           # consecutive FAILED pane captures for one window before the watcher surfaces a stale wake naming the window unreadable, instead of skipping it in silence while every pane-derived alarm for it is off; the threshold keeps a transient miss quiet and matches the stale gate's own three-capture bar; one alarm per unbroken run, re-armed by the next successful capture
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale escalates; an attributed run's step-log progress resets this timer, while stale panes whose crew is not provably working surface immediately unless they declare the pause verb
FM_PUSH_BLOCK_DWELL=90             # confirmation seconds before a native push-path block may escalate; docs/herdr-backend.md owns the behavior
...
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive provably-working stale escalations on the same pane without a busy signature before demand-deep-inspection is added
FM_WATCH_TRIAGE_LOG_MAX_BYTES=262144   # size cap for the watcher's absorbed-wake debug log
FM_FLEET_SYNC_BOOTSTRAP_TIMEOUT=     # optional seconds allowed for bootstrap's best-effort clone refresh; unset/blank defaults to max(20, 5 + 3 * origin-backed-project-count)
FM_FLEET_PRUNE=1        # set to 0 to skip pruning local branches whose upstream is gone
FM_STALE_WORKTREE_LOCK_AGE_SECS=30       # min mtime age before fm-teardown.sh treats a leftover worktree git index.lock as provably stale
FM_TREEHOUSE_RETURN_LOCK_RETRIES=3        # retries after a treehouse return fails on anything but a terminal error
...
The knob names predate the widening from Git's `Unable to create '...index.lock': File exists` signature alone to every non-terminal return failure, and are kept so existing operator settings keep working.
That lock signature still earns one extra escalation the other failures do not: teardown never removes a lock during the retry window, and after that window it attempts stale-lock cleanup only for a still-present lock that passes the configured age and live-holder checks.
Every abort also states whether re-running teardown is expected to clear the failure.
