...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/`.
  ...
  A local `config/backend` file explicitly overrides runtime auto-detection for new task endpoints and stays gitignored; spawn-supported values are `tmux` plus experimental `herdr`, `zellij`, `orca`, `cmux`, and secondmate-only `sbx`, while `codex-app` is documented only in `docs/codex-app-backend.md`.
...
- Changes to runtime session backends (`bin/fm-backend.sh`, `bin/backends/`, and the scripts that dispatch through them) need empirical adapter notes in the relevant backend guide: `docs/tmux-backend.md`, `docs/herdr-backend.md`, `docs/zellij-backend.md`, `docs/orca-backend.md`, `docs/cmux-backend.md`, `docs/sbx-backend.md`, or `docs/codex-app-backend.md` for blocked Codex App transport work.
...
bin/fm-test-run.sh --lane portable-serial   # whole portable serial remainder (watcher/AFK/tmux/stateful)
bin/fm-test-run.sh --lane portable-serial-1   # just the half CI runs on its own runner (same, still serial)
...
CI owns broad regression across required portable parallel shards, two portable serial halves on separate runners, the Herdr lane, lint, invariants, the coverage guard, and macOS snapshot compatibility in [`.github/workflows/ci.yml`](.github/workflows/ci.yml).
The serial remainder is split only across machines; each half still runs one script at a time, so stateful tests never gain a concurrent neighbour.
