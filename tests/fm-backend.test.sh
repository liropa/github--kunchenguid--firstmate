...
OLD_BIN_UNCHANGED_SIBLINGS="fm-gate-refuse-lib.sh fm-guard.sh fm-lock-lib.sh fm-state-key-lib.sh fm-tasks-axi-lib.sh fm-pr-lib.sh fm-tangle-lib.sh fm-tmux-lib.sh fm-composer-lib.sh fm-marker-lib.sh fm-wake-lib.sh fm-classify-lib.sh fm-supervision-lib.sh fm-ff-lib.sh fm-config-inherit-lib.sh fm-project-mode.sh fm-harness.sh fm-crew-state.sh fm-decision-hold.sh fm-backend.sh"
...
# The worktree is baked into both stubs: the pool hands it over on stdout (which
# is where fm-spawn.sh reads it), and the pane keeps reporting it for the
# teardown/state readers that still ask.
...
#
# docs/herdr-backend.md "Known gaps": a project can live under a symlinked
# prefix (e.g. macOS's /tmp -> /private/tmp), so fm-spawn.sh's PROJ_ABS - a
# logical `cd && pwd` - differs string-for-string from the same directory's
# physical form. The worktree pool answers with whichever form it holds, and
# validate_spawn_worktree compares the two, so a missing canonicalization would
# refuse a spawn that never tangled anything. Both forms are exercised.
...
run_spawn_symlink_case() {  # <label> <physical|logical>
  local label=$1 pool_form=$2 real_root link_root proj wt id fb data state config log out rc leased
  ...
  # /var -> /private/var), so resolve the "physical" form with pwd -P rather
  ...
  case "$pool_form" in
    physical) leased=$(cd "$wt" && pwd -P) ;;
    logical) leased=$wt ;;
    *) fail "unknown symlink pool form: $pool_form" ;;
  esac
  ...
  out=$(FM_FAKE_WORKTREE="$leased" run_spawn_case "$ROOT" "$fb" "$log" "$state" "$data" "$config" "$proj" -- "$id" "$proj" claude 2>&1)
  rc=$?
  expect_code 0 "$rc" "fm-spawn.sh should succeed for a project reached through a symlinked prefix when the pool reports a $pool_form worktree"$'\n'"$out"
  assert_contains "$out" "worktree=$leased" \
    "fm-spawn.sh did not record the leased worktree of a symlinked-prefix project when the pool reports a $pool_form path"
