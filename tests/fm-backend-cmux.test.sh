...
test_composer_state_queued_phrase_uses_recorded_harness() {
  local dir fb out harness expected
  local target='aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111'
  for harness in claude pi unset; do
    dir="$TMP_ROOT/composer-queued-$harness"; mkdir -p "$dir/responses" "$dir/state"
    fm_write_meta "$dir/state/draft.meta" 'backend=cmux' "window=$target"
    [ "$harness" = unset ] || printf 'harness=%s\n' "$harness" >> "$dir/state/draft.meta"
    cmux_panes_response "$dir" 1 'bbbbbbbb-1111-1111-1111-111111111111'
    cmux_read_screen_response "$dir" 2 '│ ❯ Press up to edit queued messages │'
    fb=$(make_cmux_fakebin "$dir")
    out=$( PATH="$fb:$PATH" FM_HOME="$dir" FM_STATE_OVERRIDE="$dir/state" FM_CMUX_LOG="$dir/log" FM_CMUX_RESPONSES="$dir/responses" \
      bash -c '. "$0/bin/fm-backend.sh"; fm_backend_composer_state cmux "$1"' "$ROOT" "$target" )
    expected=pending
    [ "$harness" != claude ] || expected=empty
    [ "$out" = "$expected" ] || fail "queued phrase on cmux harness '$harness' should read '$expected', got '$out'"
  done
  pass "fm_backend_cmux_composer_state: only recorded claude panes accept the queued phrase"
}
