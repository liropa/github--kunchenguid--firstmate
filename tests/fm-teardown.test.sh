...
add_terminal_phrase_collision_treehouse() {
  local case_dir=$1
  cat > "$case_dir/fakebin/treehouse" <<'SH'
#!/usr/bin/env bash
if [ "${1:-}" = return ]; then
  ...
  printf '%s\n' "$count" > "$count_file"
  if [ "$count" -eq 1 ]; then
    echo "failed to return worktree: git checkout --detach --force refs/remotes/origin/main: Previous HEAD position was b3aad8a fix(pool): warn when a worktree is being destroyed" >&2
    exit 1
  fi
  exit 0
fi
...
test_terminal_phrase_in_commit_subject_is_retried() {
  local case_dir rc attempt_file
  case_dir=$(make_case terminal-phrase-collision)
  write_meta "$case_dir" no-mistakes ship
  wt_commit "$case_dir" "shippable work"
  git -C "$case_dir/wt" push -q origin fm/task-x1
  git -C "$case_dir/project" fetch -q origin

  add_terminal_phrase_collision_treehouse "$case_dir"
  attempt_file="$case_dir/treehouse-attempts"
  : > "$attempt_file"
  ...
  expect_code 0 "$rc" "terminal-phrase-collision: a terminal phrase in Git output should remain retryable"
  [ "$(cat "$attempt_file")" = 2 ] \
    || fail "terminal-phrase-collision: expected exactly 2 treehouse return attempts, got $(cat "$attempt_file")"
  assert_grep "succeeded on retry" "$case_dir/stderr" \
    "terminal-phrase-collision: teardown did not report success on retry"
  pass "terminal phrases embedded in commit subjects do not suppress retries"
}
